...
final class CameraView: UIView {
    ...
    private let captureSessionQueue = DispatchQueue(
        label: "com.zaitsev.ZaitsevBank.captureSessionQueue"
    )

    // MARK: - Capture related

    private let sampleBufferQueue = DispatchQueue(
        label: "com.zaitsev.ZaitsevBank.sampleBufferQueue"
    )

    private let imageRatio: ImageRatio = .iFrame960x540
    ...
    func setupRegionOfInterest() {
        ...
        let cuttedHeight: CGFloat = cuttedWidth * CreditCard.heightRatioAgainstWidth - 20
